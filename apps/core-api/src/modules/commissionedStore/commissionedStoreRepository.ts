...
import { CommissionedStore, CommissionedStoreRowSchema } from '@modules/commissionedStore/commissionedStoreModel';

const toModel = (row: CommissionedStoreRowSchema): CommissionedStore => ({
  id: row.id,
...
export const commissionedStoreRepository = {
  getAll: async (): Promise<CommissionedStore[]> => {
    const { rows } = await knex.raw('SELECT cs.* FROM commissioned_stores cs');

    return rows.map(toModel);
  },
  get: async (id: string): Promise<CommissionedStore | null> => {
  ...
  create: async (commissionedStore: CommissionedStore): Promise<CommissionedStore> => {
  ...
  update: async (commissionedStore: CommissionedStore): Promise<CommissionedStore> => {
    const { rows } = await knex.raw(
  ...
  batchGet: async (ids: string[]): Promise<CommissionedStore[]> => {
    if (!ids.length) {
      return [];
    }
